...
var proxy = require("http").createServer((req, res) => {
    if (req.url.startsWith("/proxyto:")) {
        ...
        require(url.protocol.split(":")[0]).get(url.origin + req.url, {headers: fakeHeaders}, (resp => {
            resp.pipe(res)
        res.write(`<script>const ORIGIN_REQUEST="request";setInterval(()=>{Array.from(document.querySelectorAll("a")).forEach(r=>{r.href.startsWith("https://pearproxy.vercel.app")||(r.href="https://pearproxy.vercel.app/proxyto:"+r.href)}),Array.from(document.querySelectorAll("link")).forEach(r=>{r.href.startsWith("https://pearproxy.vercel.app")||(r.href="https://pearproxy.vercel.app/assetproxy:"+r.href)}),Array.from(document.querySelectorAll("img, audio, source, video")).forEach(r=>{r.src.startsWith("https://pearproxy.vercel.app")||(r.src="https://pearproxy.vercel.app/assetproxy:"+r.src)})},750);</script>`)
        }))
    } else {
